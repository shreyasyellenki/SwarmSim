#!/usr/bin/env bash
# Bundle J: F config + frontier 0.3 + episode horizon 1000, 300k steps.
# Hypothesis: longer episodes let sweeps finish and frontier pulls toward pockets.
set -euo pipefail

cd "$(dirname "$0")/.."
PY=.venv/bin/python
STEPS=300000
NUM_ENVS=4
ROLLOUT=256
GAMMA=0.3
CURIOSITY=0.3
FRONTIER=0.3
REPULSION=0.05
REPULSION_RADIUS=3
EPISODE_HORIZON=1000
NAME=swarm_policy_full_bundle_j

log() { echo "[$(date '+%Y-%m-%d %H:%M:%S')] $*"; }

log "=== Bundle J: frontier=${FRONTIER} horizon=${EPISODE_HORIZON}, ${STEPS} steps ==="
$PY -m swarmsim.policy.train_swarm \
  --comm-mode full \
  --timesteps "$STEPS" \
  --num-envs "$NUM_ENVS" \
  --rollout-steps "$ROLLOUT" \
  --gamma "$GAMMA" \
  --use-gru \
  --no-global-map \
  --curiosity "$CURIOSITY" \
  --frontier "$FRONTIER" \
  --repulsion "$REPULSION" \
  --repulsion-radius "$REPULSION_RADIUS" \
  --episode-horizon "$EPISODE_HORIZON" \
  --save-name "$NAME"
log "=== Finished training ==="

log "=== Evaluating deterministic ==="
$PY - <<PY
import json
from pathlib import Path
from swarmsim.policy.eval import evaluate

weights = Path("weights/${NAME}.pt")
det = evaluate(weights, comm_mode="full", deterministic=True)
out = Path("weights/bundle_j_experiment_results.json")
out.write_text(json.dumps({
    "bundle_j": {
        "revisit_gamma": ${GAMMA},
        "curiosity": ${CURIOSITY},
        "frontier": ${FRONTIER},
        "repulsion": ${REPULSION},
        "repulsion_radius": ${REPULSION_RADIUS},
        "episode_horizon": ${EPISODE_HORIZON},
        "global_map": False,
        "weights": str(weights),
        "eval_deterministic": det,
    }
}, indent=2))
print(f"Results written to {out}")
print(f"  deterministic: {det['mean_final_coverage']:.2%}")
PY

log "=== DONE ==="

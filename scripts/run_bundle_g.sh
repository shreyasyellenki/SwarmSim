#!/usr/bin/env bash
# Bundle G: Bundle F + heading diversity penalty (0.1), 250k steps.
set -euo pipefail
...
REPULSION=0.05
DIVERSITY=0.1
NAME=swarm_policy_full_bundle_g

log() { echo "[$(date '+%Y-%m-%d %H:%M:%S')] $*"; }

log "=== Bundle G: diversity=${DIVERSITY} + Bundle F rewards, ${STEPS} steps ==="
...
out = Path("weights/bundle_g_experiment_results.json")
